...
/*Iterate through each to-do item in the array:
Use a loop to go through each to-do item in the array.
Check if the to-do item is not completed:
For each to-do item, check if the completed property is false.
If not completed, log the task:
If the completed property is false, log the task property of that to-do item.
*/
...
function sortNumbers(arr: number[]): number[] {
    return arr.slice().sort((a: number, b: number) => a - b);
}
...
function logReverseArray(arr: any[]): void {
...
/* in this code me using the inquirer to get the user input and using inquirer me using 
if else statement to set the condtion to perform the addition substriction multiplicatation and 
division */
